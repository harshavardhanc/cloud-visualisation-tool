...
export default function EnhancedDashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    const runningResources = state.resources.filter(r => r.status === 'running' || r.status === 'active').length;
    const healthScore = totalResources > 0 ? Math.round((runningResources / totalResources) * 100) : 0;
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {metrics.runningResources} running, {metrics.stoppedResources} stopped
              </p>
            </CardContent>
          </Card>
          <Card>
              ...
              return (
                <Card 
                  ...
                  className={cn(
                    ...
                    account.provider === 'aws' && "bg-orange-50 dark:bg-orange-950/20 border-orange-200 dark:border-orange-800",
                    account.provider === 'azure' && "bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800",
                    account.provider === 'gcp' && "bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-800"
                  )}
                >
                  <CardHeader>
                    <div className="flex items-center justify-between">
                    ...
                    <Badge 
                      className={cn(
                        "w-fit",
                        account.status === 'connected' && "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200",
                        account.status === 'connecting' && "bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200",
                        account.status === 'error' && "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"
                      )}
                    >
              ...
              <TabsContent value="by-provider" className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {Object.entries(resourcesByProvider).map(([provider, resources]) => {
                    const providerData = providerInfo[provider as keyof typeof providerInfo];
                    const totalCost = resources.reduce((sum, r) => sum + parseFloat(r.cost.replace(/[$,\/month]/g, '')), 0);
                    ...
                    return (
                      <Card key={provider} className="p-4">
                        <div className="flex items-center space-x-3 mb-3">
                          ...
                          <div>
                            <h3 className="font-semibold">{providerData.name}</h3>
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            <span>Running:</span>
                            <span className="font-medium">{resources.filter(r => r.status === 'running' || r.status === 'active').length}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Cost:</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Object.entries(
                    ...
                    <Card key={type} className="p-4">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {resources.filter(r => r.status === 'running' || r.status === 'active').length} active
                      </p>
                    </Card>
                  ))}
                </div>
              ...
              <TabsContent value="by-status" className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  {['running', 'active', 'stopped', 'error'].map((status) => {
                    const resources = state.resources.filter(r => r.status === status);
                    return (
