...
type AppAction =
  ...
  | { type: 'UPDATE_ACCOUNT'; payload: { id: string; updates: Partial<CloudAccount> } }
...
const AppContext = createContext<{
  ...
  actions: {
    addAccount: (account: Omit<CloudAccount, 'id' | 'createdAt'>) => Promise<void>;
    removeAccount: (id: string) => Promise<void>;
    updateAccount: (id: string, updates: Partial<CloudAccount>) => Promise<void>;
  ...
  const actions = {
    ...
    removeAccount: async (id: string) => {
      ...
      try {
        ...
        const updatedResources = state.resources.filter(resource => resource.accountId !== id);
...
function generateMockResources(account: CloudAccount): Resource[] {
  const resourceTypes = {
    aws: ['EC2 Instance', 'RDS Database', 'S3 Bucket', 'Lambda Function', 'Load Balancer'],
    azure: ['Virtual Machine', 'SQL Database', 'Storage Account', 'Function App', 'Application Gateway'],
    gcp: ['Compute Engine', 'Cloud SQL', 'Cloud Storage', 'Cloud Functions', 'Load Balancer'],
  };
  ...
  for (let i = 0; i < count; i++) {
    ...
    const resource: Resource = {
      ...
      status: ['running', 'active', 'stopped'][Math.floor(Math.random() * 3)] as any,
      cpu: Math.random() > 0.3 ? `${Math.floor(Math.random() * 80) + 10}%` : undefined,
      memory: Math.random() > 0.3 ? `${Math.floor(Math.random() * 80) + 10}%` : undefined,
      network: Math.random() > 0.5 ? `${(Math.random() * 5 + 0.1).toFixed(1)} GB/s` : undefined,
      cost: `$${(Math.random() * 200 + 10).toFixed(2)}/month`,
      tags: ['production', 'development', 'staging', 'test'].slice(0, Math.floor(Math.random() * 3) + 1),
